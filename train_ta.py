...
class Recommender(object):
    ...
    def _initialize(self, interactions):
        ...
        self._net = CASTta(self._num_users,
                         self._num_items,
    ...
    def fit(self, train, verbose=False):
        ...
        for epoch_num in range(start_epoch, self._n_iter):
            ...
            # NDCG, HT = evaluate_ranking(self, train, k=[1, 5, 10], is_valid=False)
            for (minibatch_num,
                 (batch_users,
                  batch_sequences,
                  batch_targets
                ...
                item_for_prediction_encoder = torch.cat((batch_item_sequences, batch_item_sequences_t), dim=1)[:,
                                              1:].unsqueeze(2)
                cate_for_prediction_encoder = torch.cat((batch_cate_sequences, batch_cate_sequences_t), dim=1)[:,
                                              1:].unsqueeze(2)
                ...
                loss = (likelihood) + k1 * kld + likelihood1 + k2 * kld2 + likelihood2 +k3 * kld3 + likelihood_c2
                kld_loss += (kld).item()
                kld2_loss += (kld2).item()
            ...
            if verbose and (epoch_num + 1) % 1 == 0:
                ...
                torch.save(self._net, "datasets/{}/CASTta_".format(DATASET_NAME) + str(epoch_num + 1) + ".model")
            else:
    ...
    def create_batch_predict(self, user_ids, item_ids, valid_flag):
        ...
        for user_id, item_id in zip(user_ids, item_ids):
            ...
            # mask = 1.0 - np.equal(sequences_np, 0)
            # sequences_np = sequences_np[-len(mask):]
            ...
            # sequences_c, numbers_c = self.create_category_sequence(train, sequences)
            # target_c = self.create_category_sequence_for_test(train, items)
...
if __name__ == '__main__':
    ...
    log_file_name = "datasets/{}/{}_r{}_CASTta_b5_l{}_bs{}_W{}_kld1{}_kld2{}_kld3{}.txt".format(config.dataset,
                                                                                                config.dataset,
                                                                                                config.seed,
                                                                                                config.L,
                                                                                                config.batch_size,
