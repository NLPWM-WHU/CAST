...
class CASTta(nn.Module):
    ...
    def forward(self, seq_var, user_var, item_var, tim_seq_var, cat_seq_var, for_pred=False):
        ...
        h = Variable(torch.zeros(1, batch_size, self.hidden_units)).cuda()
        c = Variable(torch.zeros(1, batch_size, self.hidden_units)).cuda()
        output_ht_2, (_, _) = self.encoder1(torch.cat((tims_embs, self.change(sampled_z)), dim=2), (h,c)) # output_hc=>h^{(2)}_{c_k}
        ...
        h = Variable(torch.zeros(1, batch_size, self.hidden_units)).cuda()
        c = Variable(torch.zeros(1, batch_size, self.hidden_units)).cuda()
        outputs_hi_3, (_, _) = self.encoder3(torch.cat((item_embs, output_ht_2), dim = 2), (h, c))
        ...
        h = Variable(torch.zeros(1, batch_size, self.hidden_units)).cuda()
        c = Variable(torch.zeros(1, batch_size, self.hidden_units)).cuda()
        output_hc_2, _ = self.encoder2(torch.cat((categories_embs, self.change1(sampled_z_2)), dim=2), (h, c))
        ...
        h = Variable(torch.zeros(1, batch_size, self.hidden_units)).cuda()
        c = Variable(torch.zeros(1, batch_size, self.hidden_units)).cuda()
        output_last, _ = self.encoder4(torch.cat((item_embs, output_hc_2), dim=2), (h, c))
